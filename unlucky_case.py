...
# files = range(2, 3, 1)  # c:\dev\python\trucks\Instances\SimpleCase\Input\Data1.txt
...
DUE_TIME_FOR_TRUCK_FILE_LINE_INDEX = 32  # 32:   (30) 14	9	2	1	17	14	18	10	12	13	5	4	12	15	9	2	14	18	11	15	13	6	3	7	8	11	19	17	18	23
NUMBER_OF_PICKERS_FOR_TIME_SLOT_FILE_LINE_INDEX = 33  # 33:   (24) 8	8	8	8	8	8	8	8	8	8	8	8	8	8	8	8	8	8	8	8	8	8	8	8
...
for file_index in files:  # Data(1-20).txt
    ...
    for e in data[NUMBER_OF_ORDERS_FOR_TRUCK_FILE_LINE_INDEX]:
        ...
        ind = ind + 1
    if COLLAB == 1:
        for e in data2[NUMBER_OF_ORDERS_FOR_TRUCK_FILE_LINE_INDEX]:
            numberOfOrdersForTruck[ind] = e
            ind = ind + 1
    ...
    if COLLAB == 1:
        for ii in ORDERS_FOR_TRUCK_FILE_LINE_INDEXES:
            ...
            ind = 0
            for e in data2[ii]:
                ordersForTruck_single[ind] = e + HALF_NUMBER_OF_ORDERS
                ind = ind + 1
            if ordersForTruck_single:
                Os_ordersForTruck.append(ordersForTruck_single)
    ...
    for e in data[DUE_TIME_FOR_TRUCK_FILE_LINE_INDEX]:
        ...
        ind = ind + 1
    if COLLAB == 1:
        for e in data2[DUE_TIME_FOR_TRUCK_FILE_LINE_INDEX]:
            us_dueTimeForTruck[ind] = e
            ind = ind + 1
    ...
    # copy dataset array shifted by 1
    ind = 0
    for e in data[NUMBER_OF_PICKERS_FOR_TIME_SLOT_FILE_LINE_INDEX]:
        p_numberOfPickersForTimeSlot[ind] = e
        ind = ind + 1
    if COLLAB == 1:
        ind = 0
        for e in data2[NUMBER_OF_PICKERS_FOR_TIME_SLOT_FILE_LINE_INDEX]:
            p_numberOfPickersForTimeSlot[ind] += e
            ind = ind + 1
    ...
    # qslast = p.addVars([i for i in range(1, TOTAL_NUMBER_OF_TRUCKS + 1, 1)], vtype=gb.GRB.BINARY, name='qslast')
    ...
    for t in range(1, T_NUMBER_OF_TIME_SLOTS + 1, 1):
        p.addConstr(gb.quicksum(xot[o, t] for o in range(1, O_NUMBER_OF_ORDERS + 1, 1)) <= p_numberOfPickersForTimeSlot[t - 1], 'two')
    # p.addConstr(gb.quicksum(xot[o, 1] for o in range(1, O_NUMBER_OF_PICK_ORDERS + 1, 1)) <= 8, 'two')
    # p.addConstr(gb.quicksum(xot[o, 2] for o in range(1, O_NUMBER_OF_PICK_ORDERS + 1, 1)) <= p_pickers[1], 'two')
    ...
    if VERBOSE > 0 and p.Status not in [3, 9]:
        ...
        for s in range(1, S_NUMBER_OF_TRUCKS + 1, 1):
            if ls[s].X == ys[s].X - us_dueTimeForTruck[s - 1]:
                ...
                print(ls[s].X == ys[s].X - us_dueTimeForTruck[s - 1], ls[s].X, ys[s].X - us_dueTimeForTruck[s - 1], ys[s].X,
